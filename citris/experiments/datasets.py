...
# Dataset for structured linear temporal data
class LinearDataset(data.Dataset):
    """Dataset for structured linear temporal data"""
    
    def __init__(self, data_folder, split='train', single_image=True, return_latents=True, 
                 coarse_vars=False, img_width=None, **kwargs):
        """
        Args:
            data_folder: Path to the folder containing the NPZ files
            split: Train/val/test split
            return_latents: If True, return latent variables as labels
        """
        self.data_folder = data_folder
        self.return_latents = return_latents
        
        # Load the data - fall back to train.npz for all splits since we have one file
        data_path = os.path.join(data_folder, "train.npz")
        data = np.load(data_path)
        
        # Store observations as imgs for compatibility with CITRIS
        self.observations = torch.from_numpy(data['observations']).float()
        self.latents = torch.from_numpy(data['latents']).float()
        self.imgs = self.observations
    ...
    def target_names(self):
        """Return names of latent variables"""
        return [f'Z{i+1}' for i in range(self.latents.shape[1])]
    
    def get_img_width(self):
        """Not relevant for linear data"""
        return 1
    ...
    def get_causal_var_info(self):
        """Return information about causal variables"""
        var_info = {}
        for i, name in enumerate(self.target_names()):
            max_val = float(self.latents[:, i].abs().max()) * 1.2  # Add some margin
            var_info[name] = f"continuous_{max_val}"
        return var_info
